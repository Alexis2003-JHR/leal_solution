...
func ParseError(err error) *custom_errors.LogError {
	...
	case errors.Is(err, custom_errors.ErrDuplicatedKey):
		customErr = custom_errors.New(custom_errors.ErrDuplicatedKeyType, "User already exists", http.StatusConflict, err.Error())
	...
	case errors.Is(err, custom_errors.ErrNotFound):
		customErr = custom_errors.New(custom_errors.ErrNotFoundType, "User not found", http.StatusNotFound, err.Error())
